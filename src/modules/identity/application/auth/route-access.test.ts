...
import { ADMIN_HOME, LOGIN_PATH, RESELLER_HOME, decideRouteAccess } from "./route-access";
...
describe("decideRouteAccess", () => {
  ...
  it("lets a RESELLER into the reseller panel, and an ADMIN too", () => {
    expect(decideRouteAccess("/panel", RESELLER_CLAIMS)).toEqual({ kind: "allow" });
    expect(decideRouteAccess("/panel/catalog", ADMIN_CLAIMS)).toEqual({ kind: "allow" });
  });
